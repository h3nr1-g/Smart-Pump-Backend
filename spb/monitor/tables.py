...
class PumpTable(Table):
    ...
    class Meta:
        ...
        fields = (
            'name', 'state', 'remainingTankVolume', 'remainingBatteryCapacity', 'lastRequest', 'details', 'delete'
        )
    ...
    details = tables.columns.TemplateColumn(
        '<a href="{% url "monitor:pump_details" record.id %}">'
    ...
    delete = tables.columns.TemplateColumn(
        ...
        'onclick="delete_and_reload(\'{% url "monitor:pump_details" record.id %}\')">'
        'Delete'
        '</button>',
        verbose_name=''
    )
